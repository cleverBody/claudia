...
export const CCAgents: React.FC<CCAgentsProps> = ({ onBack, className }) => {
  ...
  const loadAgents = async () => {
    try {
      ...
      setError(t("agents.list.loadError"));
      setToast({ message: t("agents.list.loadError"), type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const confirmDeleteAgent = async () => {
    ...
    try {
      ...
      setToast({ message: t("actions.success.deleted"), type: "success" });
      await loadAgents();
      await loadRuns(); // Reload runs as they might be affected
    } catch (err) {
      console.error("Failed to delete agent:", err);
      setToast({ message: t("agents.actions.deleteAgent"), type: "error" });
    } finally {
      setIsDeleting(false);
  ...
  const handleAgentCreated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t("actions.success.created"), type: "success" });
  };

  const handleAgentUpdated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t("actions.success.updated"), type: "success" });
  };
  ...
  const handleExportAgent = async (agent: Agent) => {
    try {
      ...
      setToast({ message: t("agents.actions.exportSuccess", { name: agent.name }), type: "success" });
    } catch (err) {
      console.error("Failed to export agent:", err);
      setToast({ message: t("agents.actions.exportSuccess"), type: "error" });
    }
  };
  ...
  const handleImportAgent = async () => {
    try {
      ...
      setToast({ message: t("agents.actions.importSuccess"), type: "success" });
      await loadAgents();
    } catch (err) {
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold">{t("navigation.agents.title")}</h1>
                <p className="text-sm text-muted-foreground">
                  {t("navigation.agents.subtitle")}
                </p>
              </div>
            </div>
              ...
              <div>
                {loading ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-64 text-center">
                    ...
                    <h3 className="text-lg font-medium mb-2">{t("agents.list.empty")}</h3>
                    <p className="text-sm text-muted-foreground mb-4">
                      {t("agents.list.createFirst")}
                    </p>
                            ...
                            <Card className="h-full hover:shadow-lg transition-shadow">
                              ...
                              <CardFooter className="p-4 pt-0 flex justify-center gap-1 flex-wrap">
                                <Button
                                  ...
                                  title={t("agents.actions.runAgent")}
                                >
                                  <Play className="h-3 w-3" />
                                  {t("agents.actions.runAgent")}
                                </Button>
                                <Button
                                  size="sm"
                                  variant="ghost"
                                  ...
                                  className="flex items-center gap-1"
                                  title={t("agents.actions.editAgent")}
                                >
                                ...
                                <Button
                                  ...
                                  className="flex items-center gap-1"
                                  title={t("actions.buttons.export")}
                                >
                                ...
                                <Button
                                  ...
                                  title={t("agents.actions.deleteAgent")}
                                >
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("actions.confirm.deleteAgent", { name: agentToDelete?.name })}
            </DialogDescription>
          </DialogHeader>
